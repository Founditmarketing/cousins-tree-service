...
module.exports = async (req, res) => {
  ...
  const to   = "robertcts83@gmail.com";
  const from = process.env.CONTACT_FROM_EMAIL || "Cousins Tree Service <leads@cousinstreeservice.com>";

  const subject = `New lead from cousinstreeservice.com — ${name}`;
  const text =
`Name: ${name}
Email: ${email}
Phone: ${phone}
...
—
Sent from the contact form on cousinstreeservice.com`;
  const html = `
<h2>New lead from Cousins Tree Service</h2>
...
<p style="color:#666;font-size:13px">Sent from the contact form on cousinstreeservice.com</p>`;
